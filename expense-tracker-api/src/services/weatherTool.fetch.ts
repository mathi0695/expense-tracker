...
/**
 * Weather Tool for AI Agent (Using Native Fetch API)
 * 
 * This version uses the native fetch API (Node.js 18+) instead of axios.
  ...
  return new DynamicStructuredTool({
    ...
    func: async ({ location, days = 3, units = 'metric' }) => {
      try {
        ...
        url.searchParams.append('appid', apiKey);
        url.searchParams.append('units', units);
        url.searchParams.append('cnt', (days * 8).toString()); // API returns 3-hour intervals, 8 per day
